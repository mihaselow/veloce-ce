...
**A Rust-native control plane for HPC, CAE, and AI jobs.** Clone it, build it, and run a cluster without slurmd, Munge, or a CGI layer on `scontrol`.

Veloce is a Noise-encrypted mesh of controller, Linux workers, HTTPS API, WASM dashboard, optional Apptainer `.sif` jobs, MPI via PMI, CWL DAGs, and an optional Slurm-shaped CLI sidecar. Source-available under [BSL 1.1](LICENSE) for your own cluster—not a competing hosted scheduler.
...
| Ship a workflow | CWL DAGs, job arrays, QoS, and GRES/GPU pinning in the controller |

**Not in this repository:** a Slurm clone, Windows workers, or a hosted SaaS. Use SchedMD Slurm when you need that stack. Use Veloce when you want a scheduler you can `cargo build` and run from a laptop lab.

## Quick start

**Requirements:** Rust 1.94+, Linux, OpenSSL. [Apptainer](https://apptainer.org/) is **optional**: install it on every **worker** that runs `--image` / `.sif` jobs (`apptainer` on `PATH`). The controller and fileserver store images; they do not execute them. Full walkthrough: **[docs/quickstart.md](docs/quickstart.md)**.
...
Dashboard: `cd web && trunk build --release`. Container jobs: [docs/apptainer.md](docs/apptainer.md). Prebuilt **amd64** `.deb` / `.rpm` / `.tar.gz` (including `veloce` and `veloce-exec`) are on [GitHub Releases](https://github.com/mihaselow/veloce-ce/releases)—see [packaging/README.md](packaging/README.md).
...
| [Examples](examples/) | Sample TOML, CLI config, `hello_sleep.sh` |
...
| [`cli/`](cli/README.md) | `veloce` and `veloce-exec` |
| [`fileserver/`](fileserver/README.md) | HTTPS staging and `.sif` store |
...
Licensing and security: [hello@veloce-hpc.io](mailto:hello@veloce-hpc.io). “Veloce” is a trademark of the Licensor.
