...
Served same-origin by the Veloce web UI so Interactive Shell and VNC do not depend on CDN reachability (VPN or filtered networks).
...
Do not replace these with CDN URLs in `index.html` session-open paths.
